...
const Page = () => {
    ...
    // Check if user is a seller through page parameter for "as" keyword (/sign-in?as=)
    const searchParams = useSearchParams();
    const router = useRouter();
    const isSeller = searchParams.get("as") === "seller";

    // Redirect users back to origin page after sign-in
  ...
  const {mutate: signIn, isLoading} = trpc.auth.signIn.useMutation({
    onSuccess: () => {
        ...
        // Redirects user back to where they were if needed to sign-in
        if(origin){
            router.push(`/${origin}`);
            return;
        }
